...
module.exports.DressCode = iclass.create(Jossy, {
    ...
    constructor: function(debugMode, failOnErrors) {
        Jossy.call(this, failOnErrors);
    ...
    // Jossy's clearCache only resets Jossy's own caches. Reset the DressCode caches too,
    // otherwise a component added to the disk after a build never reaches bycname.
    clearCache: function() {
        module.exports.DressCode.superclass.clearCache.call(this);
        this._dresscodeFilePromises = {};
        this._componentsInDirPromises = {};
        this._componentsForDirPromises = {};
        this._componentsForDir = {};
    ...
    _parseCode: function(code, fname) {
        var {usages, declarations, error} = ecmaParser.parse(code);
        if (error && (this._debugMode || this._failOnErrors)) {
            console.warn(`DressCode: ${fname}: ${error.message} — syntax error, the file will be passed through unprocessed.`);
        }
        ...
        if (componentInFile) {
            declarations.forEach((declaration) => {
                if (declaration.leftPart.left === componentInFile.cname && /^[a-z]/.test(declaration.leftPart.right) && !['prototype', 'toString', 'valueOf', 'constructor'].includes(declaration.leftPart.right)) {
                    ...
                    labelPositions[startPosition] = true;
        ...
        usages.forEach((usage) => {
            ...
            var requireName = usage.cname;
            if (/^[A-Z]/.test(usage.right) && !components.bycname[cname] && /^[A-Z0-9_]+$/.test(usage.right)) {
                // An all-caps name (e.g. VERSION) is a constant-like member of a component,
                // not a component itself: include the leftmost known component of the chain.
                // If the full cname does exist in bycname (a file named in all-caps), the
                // original rule wins and this branch is skipped.
                cname = usage.left;
                requireName = usage.left;
            }
            if (components.bycname[cname]) {
                if (!injections[usage.start]) {
                    injections[usage.start] = `/*#require ${requireName}#*/`;
                } else if (!labelPositions[usage.start]) {
                    // The position is occupied by a previous declaration's endlabel,
                    // not by this declaration's own label (a self-usage of the LHS):
                    // append the require instead of dropping it.
                    injections[usage.start] += `/*#require ${requireName}#*/`;
                }
            }
        });
    ...
    _createBuilder: function(fname) {
        ...
        return new Builder(fname, components.bycname, componentInFile && componentInFile.indexfile, privatePrefix, this._debugMode ? null : this._privateNames, this._failOnErrors);
    },
        ...
        return this.getDresscodeFileContent(dirname).then((dependencies) => {
            return Promise.all(dependencies.map((dependency) => {
                return this.getComponentsInDir(dependency).then((list) => {
                    list.forEach((item) => {
                        // A cname reached through two different library paths (e.g. the same
                        // npm package, its nested and its hoisted copy) means a broken layout:
                        // fail instead of a silent last-wins. The same path listed twice in
                        // .dresscode resolves to the same objects (getComponentsInDir cache)
                        // and is skipped.
                        var existing = this._componentsForDir[dirname].bycname[item.cname];
                        if (existing && existing !== item) {
                            throw new Error(`DressCode: duplicate cname "${item.cname}": ${existing.fname} and ${item.fname}. The same library is probably reachable through several paths in .dresscode — keep only one.`);
                        }
