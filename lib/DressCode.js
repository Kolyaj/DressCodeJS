...
module.exports.DressCode = iclass.create(Jossy, {
            ...
            return Promise.all(dependencies.map((dependency) => {
                return this._getComponentList(dependency).then((list) => {
                    Object.keys(list).forEach((key) => {
                        if (list[key] == fname) {
                            privatePrefix = this._debugMode ? key.replace(/\./g, '_') : `${this.privatePrefix}${this._privatePrefixIndex.toString(36)}`;
        ...
        if (!this._dresscodeFiles[dirname]) {
            ...
            this._dresscodeFiles[dirname] = this._readFile(dresscodeFilePath).then((content) => {
                ...
                if (err.code == 'ENOENT') {
                    ...
                    return parentDirname == dirname ? [] : this._getDresscodeFileContent(parentDirname);
                } else {
                    throw err;
                }
            });
    ...
    _readFile: function(fname) {
        return this._promisify(fs.readFile, fname, 'utf8');
    },

    _readdir: function(dirname) {
        return this._promisify(fs.readdir, dirname, 'utf8');
    },
